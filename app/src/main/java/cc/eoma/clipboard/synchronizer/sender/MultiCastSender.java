...
public class MultiCastSender {
    public static void send(String multiCastAddr, Integer port, String message){
        try (MulticastSocket multicastSocket = new MulticastSocket()) {
            ...
            // 发送组播数据
            DatagramPacket hi = new DatagramPacket(message.getBytes(StandardCharsets.UTF_8), message.getBytes(StandardCharsets.UTF_8).length, InetAddress.getByName(multiCastAddr), port);
